...
var V = (e) => {
  let { devtoolsUrl: s2 } = import_react4.default.useContext(v), [m, n] = import_react4.default.useState([]), c = import_react4.default.useCallback(async () => (await (await fetch(`${s2}/api/unique-trace-items`)).json()).data, [s2]), o2 = import_react4.default.useCallback((r, p2) => {
    let t = r, h, i2, v2 = false;
    for (; !v2 && t; ) h = o(t), i2 = s(h), v2 = p2.includes(i(h) ?? ""), v2 || (t = l(t));
  ...
  }, []), d = import_react4.default.useCallback((r, p2) => {
    var v2;
    if (!r) return [];
    ...
    for (let x = 0; x < ((v2 = r == null ? void 0 : r.children) == null ? void 0 : v2.length); x++) t.push(...d(r.children[x], p2));
    return t;
  }, [o2]), a = import_react4.default.useCallback(async () => {
    let r = await c(), p2 = d(document.body, r), t = l2(p2);
    n(t);
  }, [c, d, l2]);
...
var oe = ({ element: e, name: s2, onSelect: m }) => {
  let [n] = import_react6.default.useState(() => {
    let { top: l2, left: d, width: a, height: r } = e.getBoundingClientRect(), { scrollLeft: p2, scrollTop: t } = document.documentElement, h = d + p2, i2 = l2 + t;
    return { left: h, top: i2, width: a, height: r };
  }), c = import_react6.default.useRef(null);
  import_react6.default.useEffect(() => {
    let l2 = debounce_default(() => {
      var P, N, I, L, R;
      let { top: a, left: r, width: p2, height: t } = e.getBoundingClientRect(), { scrollLeft: h, scrollTop: i2 } = document.documentElement, v2 = r + h, x = a + i2;
      (P = c.current) == null || P.style.setProperty("left", `${v2}px`), (N = c.current) == null || N.style.setProperty("top", `${x}px`), (I = c.current) == null || I.style.setProperty("width", `${p2}px`), (L = c.current) == null || L.style.setProperty("height", `${t}px`), (R = c.current) == null || R.style.setProperty("opacity", "1");
    }, 200, { leading: false, trailing: true }), d = debounce_default(() => {
      var a;
      (a = c.current) == null || a.style.setProperty("opacity", "0");
    }, 200, { leading: true, trailing: false });
    return document.addEventListener("scroll", l2), document.addEventListener("scroll", d), () => {
      document.removeEventListener("scroll", l2), document.removeEventListener("scroll", d);
    };
  }, [e]);
  let o2 = import_react6.default.useMemo(() => {
    let l2 = { width: 22, height: 22 }, d = s2.length * 7.5, a = { width: l2.width + d, height: l2.height }, r = 4;
...
var U = ({ active: e, setActive: s2, onHighlight: m, icon: n, style: c }) => {
  ...
  return import_react3.default.createElement("div", { style: c }, import_react3.default.createElement("div", { role: "button", title: "Element Selector", className: "refine-devtools-selector-button", onClick: (d) => {
    var a;
    d.preventDefault(), d.stopPropagation(), (a = document == null ? void 0 : document.activeElement) == null || a.blur(), s2((r) => !r);
  } }, n), e && import_react3.default.createElement(k, { elements: o2, onSelect: (d) => {
    m(d), s2(false);
...
var A = (e, s2) => {
  let m = { width: ne(), height: re() }, n = T(e), c = B(e), o2 = Math.min(n, (s2 ?? m).width), l2 = Math.min(c, (s2 ?? m).height);
...
var Y = ({ placement: e, visible: s2, children: m }) => {
  let [n, c] = import_react8.default.useState(false), [o2, l2] = import_react8.default.useState(null), [d, a] = import_react8.default.useState(null), [r, p2] = import_react8.default.useState(() => {
    let t = A(e);
  ...
  return import_react8.default.useEffect(() => {
    ...
    if (o2 !== null) return window.addEventListener("mouseup", t), () => {
    ...
    let t = (h) => {
      if ((o2 == null ? void 0 : o2[1]) === "x") {
        let i2 = h.clientX - ((d == null ? void 0 : d.x) ?? h.clientX), v2 = r.width + (o2 === "lx" ? -i2 : i2) * 2;
        p2((x) => ({ ...x, width: b(Math.min(T(e), Math.max(640, v2))) }));
      } else if ((o2 == null ? void 0 : o2[1]) === "y") {
        let i2 = h.clientY - ((d == null ? void 0 : d.y) ?? h.clientY), v2 = r.height + (o2 === "ty" ? -i2 : i2) * 1;
        p2((x) => ({ ...x, height: b(Math.min(B(e), Math.max(360, v2))) }));
      }
    };
    if (o2 !== null) return window.addEventListener("mousemove", t), () => {
...
var de = false ? () => null : () => {
  let [e, s2] = import_react.default.useState(false), [m, n] = import_react.default.useState(false), [c] = import_react.default.useState("bottom"), { devtoolsUrl: o2, ws: l2 } = import_react.default.useContext(v), [d, a] = import_react.default.useState(0), [r, p2] = import_react.default.useState(false), t = import_react.default.useCallback((i2) => {
    l2 && O(l2, y.DEVTOOLS_HIGHLIGHT_IN_MONITOR, { name: i2 }), n(true);
  ...
  }, [e]), e ? import_react.default.createElement("div", { style: { position: "fixed", left: `${Math.round(d / 2)}px`, transform: "translateX(-50%)", bottom: 0, zIndex: 99999 } }, import_react.default.createElement(G, { onClick: () => {
    n((i2) => !i2), p2(false);
...
var ue = false ? ({ children: e }) => e : ({ children: e }) => import_react10.default.createElement(S, null, e);
export {
  de as DevtoolsPanel,
